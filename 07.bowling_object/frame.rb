...
class Frame
  ...
  def score_and_bonus(next_frame, next_next_frame)
    ...
    if strike? && next_frame.strike? && next_next_frame
      2 * Shot::MAX_SCORE + next_next_frame.first_shot.score
    elsif strike?
      Shot::MAX_SCORE + next_frame.first_shot.score + next_frame.second_shot.score
    elsif spare?
      Shot::MAX_SCORE + next_frame.first_shot.score
    else
      score
    end
  end
